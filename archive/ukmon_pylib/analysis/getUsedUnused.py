...
def getAllImages(dtstr):
    ...
    t_beg = datetime.datetime.strptime(dtstr, '%Y%m%d').timestamp()
    t_end = t_beg + 86400

    sqlstr = f'select filname from singles where dtstamp>={t_beg} and dtstamp<={t_end}'
...
def getMatchedImageList(dtstr):
    """
    Read matched image list created on the calcengine, and load the data into MariaDB

    arguments:
    dtstr: the date for which to read the data file
    """
...
def getUncalibratedImageList(dtstr=None):
    """
    Read the match log and get a list of uncalibrated images, then load the data into MariaDB

    arguments:
    dtstr: the date for which to read the data file, defaults to today
    """
    datadir=os.getenv('DATADIR', default=os.path.expanduser('~/prod/data'))
    ...
    if os.path.isfile(logfile):
        ...
        imglist = open(os.path.join(datadir, 'single', 'used', f'uncal_{dtstr}.txt'), 'r').readlines()
        for li in imglist:
            sqlstr = f"update singles set status='U' where filname='{li.strip()}'"
            #print(sqlstr)
            cur.execute(sqlstr)
    connection.commit()
    cur.close()
