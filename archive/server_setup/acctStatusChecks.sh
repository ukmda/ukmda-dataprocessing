...
rm -f truepending.txt inflight.txt
cat pending.txt | while read i ; do grep $i withowner.txt > /dev/null ; if [ $? -ne 0 ] ; then echo $i >> truepending.txt ; else echo $i >> inflight.txt ; fi ; done
rm -f pending.txt
cat truepending.txt | while read i ; do grep $i /home/ubuntu/prod/data/reports/stationlogins.txt | cut -c43-100  ; done >> pending.txt
rm -f truepending.txt owneraware.txt
cat inflight.txt | while read i; do grep $i withowner.txt | awk -F "," '{print $1 $2 $4}' >> owneraware.txt ; done
rm inflight.txt
...
echo "Done is how many have cut over, In Flight is how many are known problems." >> statusreport.txt
echo "Pending is how many are pending cutover but no response from owner." >> statusreport.txt
echo "" >> statusreport.txt
...
echo "" >> statusreport.txt
echo "Pending List">> statusreport.txt
cat pending.txt >> statusreport.txt
echo "" >> statusreport.txt
echo "Not Uploading List">> statusreport.txt
cat not_uploading.txt >> statusreport.txt
