...
def update_route53_record(region, instance_id, dns_tag_key, dns_record_type, dns_record_ttl):
    ...
    if ('Reservations' not in ec2_desc_res) or (len(ec2_desc_res['Reservations']) < 1) or (len(ec2_desc_res['Reservations'][0]['Instances']) < 1):
        logger.warning('Error hetting EC2 instance information failure. Instance Id: ' + instance_id)
        return 
    ...
    if ('Tags' not in ec2_tags_res) or (len(ec2_tags_res['Tags']) < 1):
    ...
    ec2_tags_res = ec2.describe_tags(Filters=[
        {'Name': 'resource-id', 'Values': [instance_id]},
        {'Name': 'key', 'Values': ['DNSRecordType']}])
    if ('Tags' not in ec2_tags_res) or (len(ec2_tags_res['Tags']) < 1):
        logger.warning('EC2 instance has no Record Type. Instance Id: ' + instance_id)
    else:
        dns_record_type = ec2_tags_res['Tags'][0]['Value']
    
    """ Check DNS name """
    dns_record_names = dns_record_name.split('.')
    if (len(dns_record_names) < 3):
        logger.warning('The FQDN must over 3 levels at least. Instance Id: ' + instance_id + ', ' + dns_tag_key + ': ' + dns_record_name)
        return 
    ...
    for i in range(1, len(dns_record_names)-1):
        ...
        if (len(cur_zone_matches) > 0):
            ...
            break
    if (not zone_id):
        print('Route53 zone is not found. DNS name: ' + dns_record_name)
        return
